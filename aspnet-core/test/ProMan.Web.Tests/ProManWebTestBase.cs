...
{
    public abstract class ProManWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static ProManWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(ProManWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<ProManDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<ProManDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<ProManDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<ProManDbContext, Task<T>> func)
        {
            T result;
